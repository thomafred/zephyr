...
static adc_stm32_control_t ADCcontrol = { .initFlag = 0,
					  .isrConnectedFlag = 0,
					  .actDrv = NULL };
...
#define ADC_STM32L0_SETBITMASK(bit) (1 << (bit))
...
static int adc_stm32_read(struct device *dev, struct adc_seq_table *seq_tbl)
{
	struct adc_drvData *drvData = dev->driver_data;
	const struct adc_config *config = dev->config->config_info;
	...
	while (activeChannels) {
		if (activeChannels & 0x1) {
			if (!k_msgq_get(&ADCcontrol.adcVals, &val,
...
int adc_stm32_cfgChannel(ADC_HandleTypeDef *phadc, uint32_t c, uint32_t r)
{
...
int adc_stm32_init(struct device *dev)
{
	...
	if (!ADCcontrol.initFlag) {
		...
		k_sem_give(&ADCcontrol.adcReadSem);
		k_msgq_init(&ADCcontrol.adcVals, (char *)ADCcontrol.adcValBuff,
			    sizeof(uint16_t),
			    sizeof(ADCcontrol.adcValBuff) / sizeof(uint16_t));
...
static struct adc_config adc_config_dev0 = {
	...
	.activeChannels = 0
			  ...
			  + ADC_STM32L0_SETBITMASK(adcChannel_0)
#endif
#ifdef CONFIG_ADC0_CHAN1
			  + ADC_STM32L0_SETBITMASK(adcChannel_1)
#endif
#ifdef CONFIG_ADC0_CHAN2
			  + ADC_STM32L0_SETBITMASK(adcChannel_2)
#endif
#ifdef CONFIG_ADC0_CHAN3
			  + ADC_STM32L0_SETBITMASK(adcChannel_3)
#endif
#ifdef CONFIG_ADC0_CHAN4
			  + ADC_STM32L0_SETBITMASK(adcChannel_4)
#endif
#ifdef CONFIG_ADC0_CHAN5
			  + ADC_STM32L0_SETBITMASK(adcChannel_5)
#endif
#ifdef CONFIG_ADC0_CHAN6
			  + ADC_STM32L0_SETBITMASK(adcChannel_6)
#endif
#ifdef CONFIG_ADC0_CHAN7
			  + ADC_STM32L0_SETBITMASK(adcChannel_7)
#endif
#ifdef CONFIG_ADC0_CHAN8
			  + ADC_STM32L0_SETBITMASK(adcChannel_8)
#endif
#ifdef CONFIG_ADC0_CHAN9
			  + ADC_STM32L0_SETBITMASK(adcChannel_9)
#endif
#ifdef CONFIG_ADC0_CHAN10
			  + ADC_STM32L0_SETBITMASK(adcChannel_10)
#endif
#ifdef CONFIG_ADC0_CHAN11
			  + ADC_STM32L0_SETBITMASK(adcChannel_11)
#endif
#ifdef CONFIG_ADC0_CHAN12
			  + ADC_STM32L0_SETBITMASK(adcChannel_12)
#endif
#ifdef CONFIG_ADC0_CHAN13
			  + ADC_STM32L0_SETBITMASK(adcChannel_13)
#endif
#ifdef CONFIG_ADC0_CHAN14
			  + ADC_STM32L0_SETBITMASK(adcChannel_14)
#endif
#ifdef CONFIG_ADC0_CHAN15
			  + ADC_STM32L0_SETBITMASK(adcChannel_15)
#endif
#ifdef CONFIG_ADC0_CHAN_TEMP
			  + ADC_STM32L0_SETBITMASK(adcChannel_temp)
#endif
#ifdef CONFIG_ADC0_CHAN_VREFINT
			  + ADC_STM32L0_SETBITMASK(adcChannel_vref)
#endif
#ifdef CONFIG_ADC0_CHAN_VABT
			  + ADC_STM32L0_SETBITMASK(adcChannel_vbat)
#endif
		,
};

DEVICE_AND_API_INIT(adc_stm32, CONFIG_ADC_0_NAME, &adc_stm32_init,
		    &adc_drvData_dev0, &adc_config_dev0, POST_KERNEL,
...
DEVICE_AND_API_INIT(adc_stm32, CONFIG_ADC_1_NAME, &adc_stm32_init,
		    &adc_drvData_dev1, &adc_config_dev1, POST_KERNEL,
		    CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &api_funcs);
#endif // CONFIG_ADC_1
...
DEVICE_AND_API_INIT(adc_stm32, CONFIG_ADC_2_NAME, &adc_stm32_init,
		    &adc_drvData_dev2, &adc_config_dev2, POST_KERNEL,
		    CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &api_funcs);
#endif // CONFIG_ADC_2
